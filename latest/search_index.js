...

{
    ...
    "title": "Differences",
    "category": "section",
    "text": "Unlike matrices, an operator never reduces to a vector or a number.using LinearOperators #hide\nA = rand(5,5)\nopA = LinearOperator(A)\nA[:,1] * 3 # VectoropA[:,1] * 3 # LinearOperator# A[:,1] * [3] # ERRORopA[:,1] * [3] # VectorThis is also true for A[i,:], which returns vectors on Julia 0.6, and for the scalar A[i,j]. Similarly, opA[1,1] is an operator of size (1,1):\"(opA[1,1] * [3])[1] - A[1,1] * 3In the same spirit, the operator full always returns a matrix.full(opA[:,1])"
},
...

{
    ...
    "category": "section",
    "text": "Operators may be defined from functions. In the example below, the transposed isn\'t defined, but it may be inferred from the conjugate transposed. Missing operations are represented as nothing.dft = LinearOperator(10, 10, false, false,\n                     v -> fft(v),\n                     nothing,       # will be inferred\n                     w -> ifft(w))\nx = rand(10)\ny = dft * x\nnorm(dft\' * y - x)  # DFT is an orthogonal operatordft.\' * yBy default a linear operator defined by functions and that is neither symmetric nor hermitian will have element type Complex128. This behavior may be overridden by specifying the type explicitly, e.g.,dft = LinearOperator{Float64}(10, 10, false, false,\n                              v -> fft(v),\n                              nothing,\n                              w -> ifft(w))"
},

{
    "location": "tutorial.html#Preallocation-1",
    "page": "Tutorial",
    "title": "Preallocation",
    "category": "section",
    "text": "It is also possible to use inplace functions by preallocating the required elements. For instance:A = rand(5, 3)\nAp = Vector{Float64}(5)\nAtq = Vector{Float64}(3)\nop = LinearOperator(5, 3, false, false,\n                    p -> A * p,\n                    q -> A\' * q)\nv = op * ones(3)\nnorm(Ap - v)The same can be done with functions:op = LinearOperator(5, 3, false, false,\n                    p -> Aprod!(Ap, p)\n                    q -> Atprod!(Atq, q))where Aprod! and Atprod! return Ap and Atq, respectively."
},

{
    "location": "tutorial.html#Limited-memory-BFGS-and-SR1-1",
    "page": "Tutorial",
