...
function vcat(ops :: OperatorOrMatrix...)
  ...
  ncol = size(op, 2)
  for i = 2:nops
    size(ops[i], 2) == ncol || throw(LinearOperatorException("hcat: inconsistent row sizes"))
  end
  nrow = sum(size(ops[i], 1) for i = 1:nops)
